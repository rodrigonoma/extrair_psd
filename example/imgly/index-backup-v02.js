...
const WEIGHT_ALIAS_MAP = {
  "100": "thin", "200": "extraLight", "300": "light", regular: "normal", "400": "normal",
  "500": "medium", "600": "semiBold", "700": "bold", "800": "extraBold", "900": "heavy",
};

const TYPEFACE_ALIAS_MAP = {
  Helvetica: "Roboto", "Times New Roman": "Tinos", Arial: "Arimo", Georgia: "Tinos",
  Garamond: "EB Garamond", Futura: "Raleway", "Comic Sans MS": "Comic Neue",
};
...
async function customFontResolver(fontParameters, engine) {
  logToFile(`[CustomFontResolver] Tentando resolver a fonte: ${fontParameters.family}, style: ${fontParameters.style}, weight: ${fontParameters.weight}`);
  ...
  logToFile(`[CustomFontResolver] Fonte resolvida: ${foundFont.fontFamily}, ${foundFont.style}, ${foundFont.fontWeight}`);
  return {
    typeface,
    font: foundFont,
  };
        ...
        if (block) {
          ...
          if (blockType === '//ly.img.ubq/text' || blockType === '//ly.img.ubq/graphic') {
                  ...
                  if (typeof propValue === 'string') {
                    ...
                    if ('r' in propValue && 'g' in propValue && 'b' in propValue && 'a' in propValue) {
                      ...
                      if (propName === 'color' && blockType === '//ly.img.ubq/text') {
                        ...
                        logToFile(`[ApplyData] Cor de texto '${propName}' atualizada para: ${JSON.stringify(propValue)}`);
                        ...
                        logToFile(`[ApplyData] Cor de fundo '${propName}' atualizada para: ${JSON.stringify(propValue)}`);
                      } else {
                        // Para outras propriedades de cor genéricas, tentar setar diretamente
                        instance.block.setColor(block, propName, propValue);
                        logToFile(`[ApplyData] Propriedade de cor genérica '${propName}' atualizada para: ${JSON.stringify(propValue)}`);
                      }
                    } else {
                      // Para outros objetos (structs), tentar setStruct
                      instance.block.setStruct(block, propName, propValue);
                      logToFile(`[ApplyData] Propriedade struct '${propName}' atualizada para: ${JSON.stringify(propValue)}`);
                    }
                  } else {
                    logToFile(`[ApplyData] Tipo de dado desconhecido para propriedade '${propName}': ${typeof propValue}. Ignorando.`);
                  }
                } catch (e) {
            ...
            if (data[key].position !== undefined) {
              ...
              logToFile(`[ApplyData] Posição/Tamanho da camada '${key}' atualizado para: x=${x}, y=${y}, w=${width}, h=${height}`);
            }

          } else {
            logToFile(`[ApplyData] Camada '${key}' encontrada, mas não é do tipo texto ou gráfico. Ignorando por enquanto.`);
          }
        } else {
...
app.listen(port, () => {
  ...
  console.log(`Envie requisições POST para http://localhost:${port}/generate-image ou http://localhost:${port}/generate-images-batch`);
    ...
    for (const blockId of allBlocks) {
      ...
      if (name) { // Only process named blocks
        ...
        for (const propPath of blockProperties) {
          try {
            let value;
            // Tentar diferentes métodos get com base no tipo de propriedade
            // Esta é uma simplificação; em um cenário real, você precisaria de um mapeamento mais robusto
            if (propPath.includes('color')) {
              value = instance.block.getColor(blockId, propPath);
            } else if (propPath.includes('text') || propPath.includes('uri') || propPath.includes('name') || propPath.includes('type') || propPath.includes('identifier') || propPath.includes('format') || propPath.includes('provider')) {
              value = instance.block.getString(blockId, propPath);
            } else if (propPath.includes('size') || propPath.includes('width') || propPath.includes('height') || propPath.includes('x') || propPath.includes('y') || propPath.includes('weight') || propPath.includes('opacity') || propPath.includes('rotation')) {
              value = instance.block.getFloat(blockId, propPath);
            } else if (propPath.includes('enabled') || propPath.includes('visible') || propPath.includes('clipped') || propPath.includes('locked')) {
              value = instance.block.getBool(blockId, propPath);
            } else {
              // Tentar como string por padrão se o tipo não for óbvio
              try { value = instance.block.getString(blockId, propPath); } catch (e) {
                try { value = instance.block.getFloat(blockId, propPath); } catch (e) {
                  // Se tudo falhar, apenas logar e pular
              ...
              try {
                ...
                if (fillPropPath.includes('color')) {
                  ...
                  try { fillValue = instance.block.getString(fillId, fillPropPath); } catch (e) {
                    try { fillValue = instance.block.getFloat(fillId, fillPropPath); } catch (e) {
                      logToFile(`[ExtractData] Não foi possível extrair fill propriedade ${fillPropPath} para ${name}: ${e.message}`);
                      continue;
                    }
                  }
                }
